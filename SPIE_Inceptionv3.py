...
import os
os.chdir('E:\\SPIE_BreastPathQ_Challenge')
...
image_slide=[]
image_region=[]
for img in sorted(glob.glob("E:\\SPIE_BreastPathQ_Challenge\\datasets\\breastpathq\\datasets\\train\\*.tif")):
    image_slide.append(img[66:71])
    image_region.append(re.sub("\D", "", img[-6:-4]))
    images.append(np.array(Image.open(img))[:,:,0:3])

#Annotations
anno = pd.read_csv("E:\\SPIE_BreastPathQ_Challenge\\datasets\\breastpathq\\datasets\\train_labels.csv")
cellularity=[]
for i in range(len(images)):
    cellularity.append(float(anno.loc[operator.and_(anno['slide']==int(image_slide[i]), anno['rid']==int(image_region[i]))]['y']))
 
patient_id = pd.read_csv("E:\\SPIE_BreastPathQ_Challenge\\datasets\\breastpathq\\datasets\\patient_ids.csv")
...
# checkpoint
filepath="E:\\SPIE_BreastPathQ_Challenge\\datasets\\breastpathq\\datasets\\models\\inceptionv3_layer41_patient.hdf5"
