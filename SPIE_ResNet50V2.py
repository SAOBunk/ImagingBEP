...
import os
os.chdir('F:\\Studie\\OneDrive - TU Eindhoven\\Vakken\\2018-2019\\Kwart 4\\BEP')
...
image_slide=[]
image_region=[]
for img in sorted(glob.glob("F:\\Studie\\OneDrive - TU Eindhoven\\Vakken\\2018-2019\\Kwart 4\\BEP\\datasets\\train\\*.tif")):
    image_slide.append(img[66:71])
    image_region.append(re.sub("\D", "", img[-6:-4]))
    images.append(np.array(Image.open(img))[:,:,0:3])

#Annotations
anno = pd.read_csv("F:\\Studie\\OneDrive - TU Eindhoven\\Vakken\\2018-2019\\Kwart 4\\BEP\\datasets\\train_labels.csv")
cellularity=[]
for i in range(len(images)):
    cellularity.append(float(anno.loc[operator.and_(anno['slide']==int(image_slide[i]), anno['rid']==int(image_region[i]))]['y']))
 
patient_id = pd.read_csv("F:\\Studie\\OneDrive - TU Eindhoven\\Vakken\\2018-2019\\Kwart 4\\BEP\\datasets\\patient_ids.csv")
...
# checkpoint
filepath="F:\\Studie\\OneDrive - TU Eindhoven\\Vakken\\2018-2019\\Kwart 4\\BEP\\datasets\\models\\ResNet50V2.hdf5"
checkpoint = ModelCheckpoint(filepath, monitor='val_mean_squared_error', verbose=1, save_best_only=True, mode='min')
tensorboard = TensorBoard(log_dir='./logs', histogram_freq=0, write_graph=True, write_images=False)
callbacks_list = [checkpoint, tensorboard]
   
# train the model on the new data for a few epochs
model.fit_generator(datagen.flow(images[trainind], np.reshape(cellularity[trainind], (-1,1)), batch_size=10, shuffle=True), steps_per_epoch=100, epochs=100, validation_data=val_datagen.flow(images[valind], np.reshape(cellularity[valind], (-1,1)), batch_size=10, shuffle=False), validation_steps=22, callbacks=callbacks_list)
...
nuclei = pd.read_csv("E:\\SPIE_BreastPathQ_Challenge\\datasets\\breastpathq\\datasets\\nuclei_results.csv", sep='\s*,\s*')
